...
export function CatalogPreviewSection({
  ...
  const [reviewStats, setReviewStats] = useState<ProductReviewStats>(reviewStatsProp);
  ...
  return (
    <div style={styles.wrap}>
      <button
        ...
        aria-label={t(lang, "catalogViewAll")}
      >
        <span>{t(lang, "catalogPreviewTitle")}</span>
      ...
      <div style={styles.grid}>
        ...
        <div style={styles.rightColumn}>
          {rest.map((p) => renderCard(p, styles.cardSmall))}
        </div>
      </div>
    </div>
  );
