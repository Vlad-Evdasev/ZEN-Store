...
bot.command("start", async (ctx) => {
  ...
  await ctx.reply(lines.join("\n"), {
    parse_mode: "HTML",
    link_preview_options: { is_disabled: true },
    // Активно убираем reply-keyboard, если она осталась у юзера после
    // прошлых версий /start. Каталог открывается через menu_button
    // «Shop» — он передаёт WebApp полный signed initData. Прямой
    // web_app в reply-keyboard работал бы без initData (см.
    // https://core.telegram.org/bots/webapps) и ломал requireOwnership
    // под TG_AUTH_STRICT=1.
