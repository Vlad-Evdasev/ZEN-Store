...
**Примечание к Task 1+2:** В `frontend/tsconfig.json` включены `noUnusedLocals` и `noUnusedParameters`, поэтому пустой каркас с неиспользуемыми иконками и props не собирается. Task 1 и Task 2 объединены в одну задачу «Task 1+2: Полный компонент HeaderArcMenu» — одним коммитом создаём файл с props, иконками и полным рендером (overlay + кружки по дуге).
